...
public class ItemPactTest {
	...
	public RequestResponsePact createPact(PactDslWithProvider builder) throws IOException {
		...
		return builder.given("Items list").uponReceiving("request for an item").path("/item-storage/items/1234")
				.method("GET").headers(headers).willRespondWith().status(200).body(itemBody).toPact();
	}
	...
	public void pactTest(TestContext context) {
		...
		router.route("/circulation/loans").handler(routingContext -> {
			...
			System.out.println(handler.getBodyAsString() + "failing");

		});
		server.requestHandler(router::accept).listen(8080, "localhost", response -> {

			client.getAbs("http://localhost:8080/circulation/loans").putHeader("x-okapi-url", provider.getUrl())
					.putHeader("x-okapi-token", "token").handler(handler -> {
					}).end();
		});
